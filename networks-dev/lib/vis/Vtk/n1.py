...
class VtkNetwork:
    ...
    def _initialize(self,G):
        ...
        self.radius.SetNumberOfTuples(self.vtkPoints.GetNumberOfPoints())
        self.radius.SetName('Radius')
        
        for i in xrange(self.vtkPoints.GetNumberOfPoints()):
            self.radius.SetTuple1(i,1 + 1/(1.0 + i))
    ...
    def setPointWidthByID(self,point_id,width):
        
        self.vtkPolyData.GetPointData().GetArray('Radius').SetTuple1(point_id,width)
